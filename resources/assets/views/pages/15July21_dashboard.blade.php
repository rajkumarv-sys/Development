...
<div id="mymodal-image" class="modal fade " tabindex="-1" role="dialog" data-keyboard="false" data-backdrop="static">
 ...
 <form action="{{url('/outlet/store')}}" id="outlet" method="post" enctype="multipart/form-data">
  ...
  <div class="col-md-6 grid-margin">
          ...
          <div class="form-group ">
            ...
            <option selected="" disabled selected hidden>Select Channel</option>
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="sub_channel_name" name="sub_channel_name">
              <option selected="" disabled selected hidden>Select Subchannel</option>
...
<div id="mymodal-filter" class="modal fade" tabindex="-1" role="dialog" data-keyboard="false" data-backdrop="static">
    ...
    <script>
      ...
      var locationcontrol = L.control.locate({
        position: 'bottomright',flyTo:false,keepCurrentZoomLevel:true,setView:false,drawMarker:true,showCompass:true,
      ...
      map.on('locationfound', function (e) {
       ...
       var findway = L.easyButton({
        ...
        states: [{
          ...
          icon: '<i class="fa fa-filter"></i><i class="fa fa-times"></i>',
          title: 'Navigate',
          onClick: function (control) {
            showboundbyusertype('');
          }
      ...
      decryptMessage: function(encryptedMessage = '', mapkeystr){
        ...
        function loadmap(res, view) {
            ...
            for (i = 0; i < res['maplist'].length; i++) {
                ...
                var decrypted = JSON.parse(code.decryptMessage(message, jqxhr));
        ...
        function changeproperty(res,type) {
           ...
           if(type == 4)
           {
                  ...
                  $.each(res['map_nextlevel_info'], function(key, value) {                       
                    ...
                    {
                           info='';
                        //    info='<img src= width="50px"/><b>Shop </b>'+value.outlet_name+'<br><b>Channel </b>'+value.channel_name+'<br><b>Subchannel </b>';
                        info='<div class="media outlet-list"><img class="align-self-start" src="'+value.shop_image+'" width="100px" alt=""><div class="media-body"><ul class="list-group"><li class="list-group-item"><h3>'+value.outlet_name+'</h3></li><li class="list-group-item"><span>Channel:</span>'+value.channel_name+' </li><li class="list-group-item"><span>Subchannel:</span>'+value.sub_channel_name+'</li></ul></div></div>';
                          if(value.icon !='' && value.icon!==undefined)
                          {
                             ...
                             var greenIcon = L.icon({
                              ...
                              var m = new L.Marker(new L.LatLng(value.lat,value.lon),{icon: greenIcon});
                          ...
                          else
                          {
                               var m = new L.Marker(new L.LatLng(value.lat,value.lon));
                                ...
                                m.bindPopup(info,{'sticky': true,pane: 'tool',direction:'top'});
