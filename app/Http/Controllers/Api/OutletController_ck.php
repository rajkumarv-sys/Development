...
class OutletController extends Controller
{
    ...
    public function hri_delhi_outlets(Request $request)
    {
     $input = $request->all();
       ...
       $uncovered=DB::table("ckpl_uncvrd_outlets_test")->select( "refid", "retailer_id", "name", "type", "sub_type", "outlet_potential", "locality_name", "nbhrd", "address", "latitude", "longitude", "contact", "beat_id", "icon", "shop_image", "status",DB::raw("if(status='N','New',if(status='A','Found',if(status='NF','Not Found',''))) as outlet_status"),DB::raw("if(fld1923=3,'High',if(fld1923=2,'Medium',if(fld1923=1,'Low',''))) as outlet_potential_status"),"loc15","loc16", "beat_id", "fld1923", "city_id", "city", "nbhrd as beat_name","app_status");
      ...
      public function hri_uncovered_outlets(Request $request)
    {
      ...
      // $covered_sql="SELECT `refid`,  `loc12`,`beat_id`, `loc13`, `loc14`, `loc15`, `loc16`, `loc25`, `mapping_id`, `division`, `zone`, `state`, `mapped_user_name`, `fld2020`, `desination`, `fld2018`, `beat_name`, `area_name`, `name`, `latitude`, `longitude`, `fld2019`, `outet_classification`,  `city_name`,channel_type,address  FROM `hri_mumbai_andheri_outlet_master` where stat='A' and latitude!=0 and longitude!=0 and beat_id in (".implode(',',$beat_list).") order by refid asc";
       ...
       $covered_sql="SELECT `refid`,  `loc12`,loc15 as `beat_id`, `loc13`, `loc14`, `loc15`, `loc16`,  `zone`, `state`, `mapped_user_name`, `fld2020`, `desination`, `fld2018`, `beat_name`, `area_name`, `name`, `latitude`, `longitude`, `outet_classification`,  `city_name`,channel_type,address  FROM `ckpl_mumbai_outlet_master` where stat='A' and latitude!=0 and longitude!=0 and loc15 in (".implode(',',$beat_list).") order by refid asc";
       ...
       $uncovered=DB::table("ckpl_uncvrd_outlets_test")->select( "refid", "retailer_id", "name", "type", "sub_type", "outlet_potential", "app_status","fld1923", "address", "contact", "latitude", "longitude", "city","city_id",   "nbhrd", "locality_name", "loc15 as beat_id","nbhrd as beat_name", "rd_code", "rd_name", "beat_choco_consmptn", "beat_biscuit_consmptn", "beat_confec_consmptn", "beat_premium_index", "beat_snacking_index", "loc16","status","stat",DB::raw("if(status='N','New',if(status='A','Found',if(status='R','Not Found',''))) as outlet_status"),"icon","shop_image",DB::raw("if(fld1923=3,'High',if(fld1923=2,'Medium',if(fld1923=1,'Low',''))) as potential_status"))->whereIn("loc15",$beat_list);
      ...
      if(isset($input['outlet_type']))
       ...
       for($k=0;$k<$uncovered_count;$k++)
       {
            ...
            $temp['info']=[];
             ...
             array_push($temp['info'],['key'=>'Name','value'=>'rega','type'=>'text']);
             
             array_push($temp['info'],['key'=>'Neighborhood','value'=>'Test','type'=>'text']);
             array_push($temp['info'],['key'=>'City','value'=>'Test','type'=>'text']);
             array_push($temp['info'],['key'=>'Sub-Channel','value'=>'Test','type'=>'text']);
             array_push($temp['info'],['key'=>'Address','value'=>'dsafcdsvdsv','type'=>'text']);
             array_push($temp['info'],['key'=>'Contact','value'=>'9898888899','type'=>'phone']);
             array_push($temp['info'],['key'=>'Outlet Potential','value'=>'Low','type'=>'text']);
