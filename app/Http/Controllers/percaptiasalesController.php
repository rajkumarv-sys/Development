...
class percaptiasalesController extends Controller
{
              ...
              for($k=0;$k<$pop_count;$k++)
               {
                  ...
                  $population[$pop_sql_result[$k]['locid']]['period']=$pop_sql_result[$k]['period'];
    ...
    public static function percaptia($details,$maparray,$input_obj)
    {
       ...
       //   $consmptn_result=ConsmptionController::consmption($details,$maparray,$input_obj,true);
         ...
         if($details['main_location']!=$details['sub_location'])     
          ...
          $pop_result=self::consmption($details,$maparray,$input_obj);
        ...
        //  if($details['main_location']==$details['sub_location'] && $details['main_location']==12)
            ...
            $additional_result['column'][3]=array('src'=>'premium_result','type'=>'I','align'=>'right','label'=>'Mdlz Sales (Rs.)');
              $additional_result['column'][4]=array('src'=>'result','type'=>'I','align'=>'right','label'=>'Per Capita Sales (Rs.)');
        ...
        //   $additional_result['column'][5]=array('src'=>'result','type'=>'I','align'=>'right','label'=>'Penetration %');
            ...
            for($j=0;$j<count($location_wise_result);$j++)
             {    
               ...
               //   $final_result[$i]['result']=$final_result[$i]['result']+$location_wise_result[$j]['result'];
                  ...
                  // echo $filter_id.'-'.$location_wise_result[$j]['period_Y'];die;
            
            
                }   
                if(isset($pop_result[$filter_id]))
                 {
                   $final_result[$i]['population']=$pop_result[$filter_id]['pop_data'];
                     $final_result[$i]['result']=round((($final_result[$i]['premium_result']/$pop_result[$filter_id]['pop_data'])),0);
                 }         
         ...
         if(isset($details['maparray'][$final_result[$i]['loc_id']]))
           $additional_result['values'][$final_result[$i]['loc_id']]=array('loc_id'=>$final_result[$i]['loc_id'],'location_name'=>$details['maparray'][$final_result[$i]['loc_id']]['location_name'],'result'=>$final_result[$i]['result'],'percapita_result'=>number_format($final_result[$i]['result'],0),'totalsales_result'=>number_format($final_result[$i]['totalsales_result'],0),'premium_result'=>number_format($final_result[$i]['premium_result'],0),'pop_data'=>number_format($pop_result[$filter_id]['pop_data'],0),'population'=>$population[$final_result[$i]['fld1225']]);

        
         }
            
             $details['tooltip']=['Total Indvdls'=>['pop_data','Indvdls',0],'Mdlz Sales'=>['premium_result','Rs.',0,1],'Per Capita Sales'=>['percapita_result','Rs.',0,1]];
