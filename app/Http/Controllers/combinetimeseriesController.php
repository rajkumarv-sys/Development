...
class CombinetimeseriesController extends Controller
{
    private $low=[5,69,54];
    ...
    public function Combinetimeseries($result,$detailed_result,$additional_result,$suffix_of_result)
    {
         ...
         array_multisort($result_value, SORT_DESC, $result);
...
//var_dump($result);die;
      ...
      $info_text .='</div>';
          ...
          $temp=array('location_id'=>$result[$k]['loc_id'],'location_name'=>$detailed_result['maparray'][(string)$result[$k]['loc_id']]['location_name'],'result'=>round($result[$k]['result'],2),'rank'=>($k+1).'/'.$totalcount,'color'=>$color,'info'=>$info_text,'period'=>$additional_result['values'][$result[$k]['loc_id']]['period'],'size'=>$size); 
     ...
     //  array_push($data['chartdata'],['label'=>'Timeseries Chart','function_name'=>'timelinechart','structure'=>self::timelinechart($data['result'],$detailed_result,$format['chartsymbol'])]);
    ...
    public static function timelinechart($processed_result,$detailed_result,$formatsymbol)
	{
          ...
          $year=$detailed_result['year'];
	       ...
	       // $bubble_chart['title']['text']=$detailed_result['head'];
	        ...
	        $timelinechart['xAxis']['categories']=$year;
	       ...
	       for($i=0;$i<count($processed_result);$i++)
          {
            ...
            $temp=[];
            $temp['name']=$processed_result[$i]['location_name'];
            $temp['data']=array_values($processed_result[$i]['period']);
            $temp['info']=$processed_result[$i]['info'];
            $temp['color']=$processed_result[$i]['color'];
            $temp['marker']=['fillColor'=>$processed_result[$i]['color']];
            array_push($timelinechart['series'],$temp);
  ...
  public function gettable($processed_result,$detailed_result,$table_result,$suffix_of_result)
  {
    ...
    if($detailed_result['calendar_type']==4)
     ...
     $maketbl='';
      ...
      {
          $v['colspan']=(isset($v['colspan'])) ? $v['colspan'] :0;
            ...
            if($column[$i]['src'] =='')
             {
                ...
                for($y=0;$y<count($detailed_result['year']);$y++)
                {
                   ...
                   $content_tbl.='<td align="'.$column[$i]['align'].'">'.number_format(round($v['period'][$period],2),1).'</td>';
                }
             }
             else
               $content_tbl.='<td align="'.$column[$i]['align'].'">'.number_format(round($v[$column[$i]['src']],2),1).'</td>';
          }
 ...
 //      $maketbl.='<thead><tr>';
 //      $maketbl.='<td>#</td>';
 //      $maketbl.='<td align="right">'.$detailed_result['loc_axis'].'</td>';
 //       foreach($column as $k=>$v)
 //      {
 //          $maketbl.='<th align="right" colspan="'.$v['colspan'].'">'.$v['label'].'</th>';
 //      }      
 ...
 //          $content_tbl .='<tr>';
 //          $content_tbl.='<td align="right">'.$count.'</td>';
 //          $content_tbl.='<td align="left">'.$v['location_name'].'</td>';
 //          for($i=0;$i<count($column);$i++)
 //          {
