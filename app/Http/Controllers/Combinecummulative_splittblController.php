...
class Combinecummulative_splittblController extends Controller
{
    private $low=[5,69,54];
    ...
    public function Combinecummulative_splittbl($result,$detailed_result,$additional_result,$suffix_of_result)
    {
          ...
          if(isset($result[$k]['location_type']) && $result[$k]['location_type'] != '')
          {
              if($result[$k]['location_type']=='village')
              {
                   ...
                   else
                   {
                     ...
                     $label='<div class="range-label"><span>High</span><span>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;Low</span></div>';
   ...
   if(isset($detailed_result['maparray'][(string)$result[$k]['loc_id']]))
      ...
      $info_text .='</div>';
      $info_text1='<div class="container-fluid p-3 popupbox" id="ttpm" style="z-index: 1000;height:fit-content;color:white !important;"><span class="d-flex flex-row justify-content-between"><h6 style="padding-top:0.3rem;max-width: 60%;">'.$detailed_result['maparray'][(string)$result[$k]['loc_id']]['location_name'].' &nbsp;</h6><div style="height:max-content;margin-right: -0.7rem;"><img class="ml-1"  src="icons/share-icon.png" height="30px"  id="share_'.$result[$k]['loc_id'].'" onclick="share(\''.$temp['shareinfo'].'\',this)"  lat="'.$detailed_result['maparray'][(string)$result[$k]['loc_id']]['latitude'].'" lon="'.$detailed_result['maparray'][(string)$result[$k]['loc_id']]['longitude'].'"><img class="ml-1" geocode="'.$detailed_result['maparray'][(string)$result[$k]['loc_id']]['latitude'].','.$detailed_result['maparray'][(string)$result[$k]['loc_id']]['longitude'].'" onclick="location_navigate(this)" src="icons/navigation-icon.png" height="30px"><span class="popup" style="background: transparent;"><img class="ml-1"  src="icons/close-icon.png" height="30px" id="'.$result[$k]['loc_id'].'" onClick="closeicon_map(this)"></span></div></span>'.$rank_info.'<div id="rem_lddis"><hr style="border-top: 1px solid white;">'.$info_text;
    } 
       ...
       if($detailed_result['main_location']==16 && $detailed_result['sub_location']==16)
       {
        $data['icon_data']=[];
            if(count($result) > 0)
            {
                 foreach($additional_result['values'] as $k=>$v)
                 {

                    $retailer_count=count($v['subarray']);
                    for($i=0;$i<$retailer_count;$i++)  {
                         $style_code='';

                 if($v['subarray'][$i]['fld1923']==1)
                  $style_code='background-color:#51c82c';
                 if($v['subarray'][$i]['fld1923']==2)
                  $style_code='background-color:#ed8102';
                 if($v['subarray'][$i]['fld1923']==3)
                  $style_code='background-color:#bf1414';
                        $popup_text='<div class="container-fluid p-3 popupbox" id="ttpm" style="z-index: 1000;height:fit-content;color:white !important;"><span class="d-flex flex-row justify-content-between"><h6 style="padding-top:0.3rem;max-width: 60%;">'.$v['subarray'][$i]['name'].' &nbsp;</h6><div style="height:max-content;margin-right: -0.7rem;"><img class="ml-1"  src="icons/share-icon.png" height="30px"><img class="ml-1" geocode="'.$v['subarray'][$i]['latitude'].','.$v['subarray'][$i]['longitude'].'" onclick="location_navigate(this)" src="icons/navigation-icon.png" height="30px"><span class="popup" style="background: transparent;"><img class="ml-1" id="tcls" src="icons/close-icon.png" height="30px"></span></div></span><h5><span class="badge font-italic" style="text-align: left;background-color:#424242;"><span style="line-height:1rem;">'.$v['subarray'][$i]['state'].' State</span><br><span style="line-height:1rem;">'.$v['subarray'][$i]['district'].' District</span><br><span style="line-height:1rem;">'.$v['subarray'][$i]['city'].' City</span></span></h5>';
                    if($v['subarray'][$i]['shop_image']!='')
                       $popup_text .='<img class="align-self-start" style="margin-top:1rem;" src="'.$v['subarray'][$i]['shop_image'].'" width="auto" alt="" height="150px"/>';
                   $popup_text .='<div id="rem_lddis"><hr style="border-top: 1px solid white;"><p><span style="color:rgb(242, 101, 34);font-weight:bold;">Channel Type: </span>'.$v['subarray'][$i]['subtype'].' </p><p><span style="color:rgb(242, 101, 34);font-weight:bold;">Address: </span>'.$v['subarray'][$i]['address'].' </p><p><span style="color:rgb(242, 101, 34)">Outlet Potential: </span><span style="'.$style_code.'">'.$v['subarray'][$i]['potential'].'</span> </p></div>';
                        array_push($data['icon_data'],array('title'=>$popup_text,'latitude'=>$v['subarray'][$i]['latitude'],'longitude'=>$v['subarray'][$i]['longitude'],'split_id'=>$v['subarray'][$i]['split_id'],'icon'=>$v['subarray'][$i]['icon'],'type'=>$v['subarray'][$i]['subtype']));
                    }
       ...
       // $legend ='<div class="col-md-12">'.$label.'<div class="combine-color"><i style="flex: 1;-webkit-box-flex: 1;margin: 1px 1px;text-align: left;font-size: 1em;line-height: 1em;display: block; width: 100px;height: 10px;'.$legend.';"></i><span>'.$detailed_result['menu_axis'].'</span></div></div>';
        ...
        $legend ='<div class="col-md-12">'.$label.'<div class="combine-color"><i style="flex: 1;-webkit-box-flex: 1;margin: 1px 1px;text-align: left;font-size: 1em;line-height: 1em;display: block; width: 100px;height: 10px;'.$legend.';"></i></div><hr style="background-color:#fff; margin:2px;margin-top: 5px;"><p><span style="color:#fff;font-weight:bold;margin: 0px;">Catgry: </span><font style="color:rgb(242, 101, 34);">'.implode(",",$detailed_result['title']).'</font> </p><p><span style="color:#fff;font-weight:bold;margin: 0px;">Period: </span><font style="color:rgb(242, 101, 34);">'.implode(",",$detailed_result['year']).' </font></p><p><span style="color:#fff;font-weight:bold;margin: 0px;">Metric: </span><font style="color:rgb(242, 101, 34);">'.$detailed_result['menu_axis'].'</font></p></div>';
       array_push($data['legend'],$legend);
	   ...
	   public function gettable($processed_result,$detailed_result,$table_result,$suffix_of_result)
  {
     ...
     foreach ($column_value as $k => $v) {
             ...
             $child=htmlspecialchars(json_encode($v['subarray']), ENT_QUOTES, 'UTF-8');
             
              $content_tbl .='<tr id="show_data" json_attr="'.$child.'">';

              $content_tbl.='<td align="right" class=" dt-control">'.$count.'</td>';
              $content_tbl.='<td><a href="#" id="'.$v['loc_id'].'" style="text-decoration:underline;" onClick="showbound(this)">'.$v['location_name'].'</a></td>';
              $m=0;$column_length=count($column);
              foreach ($column as $key => $value) {
              
                  if($m==($column_length-1))
                    $content_tbl.='<td align="right">'.count($v['subarray']).'</td>';
                  else
                    $content_tbl.='<td>'.$v[$value['src']].'</td>';
