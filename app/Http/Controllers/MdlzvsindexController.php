...
class MdlzvsindexController extends Controller
{
    ...
    public static function mdlzindex($details,$maparray,$input_obj){
       ...
       $final_result=[];
        ...
        $val=[186=>1,187=>2];
         ...
         $parent_id=array_unique(array_column($input_obj['menu_list'],'parent_id'));
        //$details['year']=[2022];

       $select_criteria=[];$where=[];$temp=''; $head='';$select=[];
       
       $sql=DB::table('nbhrd_premium_index_master')->whereIn('refid', [$val[$parent_id[0]]])->select(['refid','name'])->get();  
         ...
         if($details['main_location']!=$details['sub_location'])     
          ...
          if(count($index_type)>0)
            array_push($where,' a.fld1955  in ('.$val[$parent_id[0]].')');
    ...
    public static function premium_sales($details,$maparray,$input_obj)
    {
       ...
       $index=$query." order by period asc,result desc";
          ...
          $mdlz_index=self::mdlzindex($details,$maparray,$input_obj);
          $m_count=count($mdlz_index);
          $mdlz_index_array=[];
          for($m=0;$m<$m_count;$m++)
          {
            $mdlz_index_array[$mdlz_index[$m]['loc_id']]=$mdlz_index[$m];
          }
          ...
          $totalresult=array_sum(array_column($prmium_sales_result,'result'));
          $prmium_sales_result_count=($prmium_sales_result_count<=0) ? 1 :$prmium_sales_result_count;
          if($details['main_location']!=$details['sub_location'])     
            $avg_value=$totalresult/$prmium_sales_result_count;
          else{
            $count_val=self::getsublevelcount($details['main_location'],$details['sub_location'],$details['selected_location']);
            $avg_value=$totalresult/(($count_val>0)?$count_val:1);
          }
          ...
          if($details['main_location']==$details['sub_location'] && $details['main_location']==12)
            $additional_result['column'][2]=array('src'=>'population','type'=>'I','align'=>'left','label'=>'Popn Strata');
    $si_pi='';
    $ss_ps='';
     if(!in_array(6,$details['cndn_value']))
     {
        $additional_result['column'][3]=array('src'=>'mdlz_sales','type'=>'I','align'=>'right','label'=>'MDLZ Premium Sales Rating');
        $additional_result['column'][4]=array('src'=>'mdlz_score','type'=>'I','align'=>'right','label'=>'Premium Index Rating');
        $additional_result['column'][5]=array('src'=>'premium_result','type'=>'I','align'=>'right','label'=>'MDLZ  Sales (Rs.)');
        $additional_result['column'][6]=array('src'=>'score_result','type'=>'I','align'=>'right','label'=>'Premiumness Score');
        $additional_result['column'][7]=array('src'=>'score_index','type'=>'S','align'=>'left','label'=>'Premium Index');
        $si_pi='PI'; $ss_ps='PS';
     }
     else
     {
        $additional_result['column'][3]=array('src'=>'mdlz_sales','type'=>'I','align'=>'right','label'=>'MDLZ Biscuit Sales Rating');
        $additional_result['column'][4]=array('src'=>'mdlz_score','type'=>'I','align'=>'right','label'=>'Snacking Index Rating');
        $additional_result['column'][5]=array('src'=>'premium_result','type'=>'I','align'=>'right','label'=>'MDLZ Sales (Rs.)');
        $additional_result['column'][6]=array('src'=>'score_result','type'=>'I','align'=>'right','label'=>'Snacking Score');
        $additional_result['column'][7]=array('src'=>'score_index','type'=>'S','align'=>'left','label'=>'Snacking Index');
        $si_pi='SI';  $ss_ps='SS';
     }
           ...
           for($i=0;$i<$filter_count;$i++)
          {
            ...
            for($j=0;$j<count($location_wise_result);$j++)
             ...
             $final_result[$i]['premium_result']=round($final_result[$i]['premium_result'],1);
             $final_result[$i]['loc_id']=$filter_id;
             $final_result[$i]['score_result']=0;
             if(isset($mdlz_index_array[$filter_id]))
             {
              $final_result[$i]['score_result']=round($mdlz_index_array[$filter_id]['score_result'],1);
              $final_result[$i]['score_index']=$mdlz_index_array[$filter_id]['result'];
               $final_result[$i]['population']=$mdlz_index_array[$filter_id]['popltn'];


             }
             $mdlz_scale=['index_score'=>30,'sales'=>$avg_value];
             $mdlz_score=''; 
             $mdlz_sales='';
              if(!in_array(6,$details['cndn_value']))  {
                $si_pi='PI';$ss_ps='PS';
             ...
             if(($final_result[$i]['score_result'] > $mdlz_scale['index_score']) && ($final_result[$i]['premium_result'] > $mdlz_scale['sales']))
            {
                 
                $mdlz_score='High';
                $mdlz_sales='High'; $color="#01875b";
                $potential='High '.$si_pi.' / High '.$ss_ps;
            }
                
            if(($final_result[$i]['score_result'] < $mdlz_scale['index_score']) && ($final_result[$i]['premium_result'] < $mdlz_scale['sales']))
            {
                $mdlz_score='Low';
                $mdlz_sales='High'; $color="#f2020a";$potential='Low '.$si_pi.' / High '.$ss_ps;
            }
            if(($final_result[$i]['score_result'] > $mdlz_scale['index_score']) && ($final_result[$i]['premium_result'] < $mdlz_scale['sales']))
            {
                $mdlz_score='High';
                $mdlz_sales='Low';$color="#fcff00";$potential='High '.$si_pi.' / Low '.$ss_ps;
            }
            if(($final_result[$i]['score_result'] < $mdlz_scale['index_score']) && ($final_result[$i]['premium_result'] > $mdlz_scale['sales']))
            {
                $mdlz_score='Low';
                $mdlz_sales='Low'; $color="#ff7f02";$potential='Low '.$si_pi.' / Low '.$ss_ps;
            }
             $final_result[$i]['color']=$color; 
             $final_result[$i]['mdlz_score']=$mdlz_score; 
             $final_result[$i]['mdlz_sales']=$mdlz_sales; 
             $final_result[$i]['potential']=$potential;  
         if(isset($details['maparray'][$final_result[$i]['loc_id']]))
           $additional_result['values'][$final_result[$i]['loc_id']]=array('loc_id'=>$final_result[$i]['loc_id'],'location_name'=>$details['maparray'][$final_result[$i]['loc_id']]['location_name'],'premium_result'=>number_format($final_result[$i]['premium_result'],0),'score_result'=>number_format($final_result[$i]['score_result'],0),'score_index'=>$final_result[$i]['score_index'],'mdlz_score'=>$mdlz_score,'mdlz_sales'=>$mdlz_sales,'population'=>$population[$final_result[$i]['fld1225']],'color'=>$color);

        
         }

         if(!in_array(6,$details['cndn_value']))   
             $details['tooltip']=['MDLZ Premium Sales'=>['premium_result','Rs.',0,1],'Premiumness Score'=>['score_result','',0],'Premium Index'=>['score_index','',0]];
         else
            $details['tooltip']=['MDLZ Biscuit Sales'=>['premium_result','Rs.',0,1],'Snacking Score'=>['score_result','',0],'Snacking Index'=>['score_index','',0]];
    ...
    public static function getsublevelcount($main_location,$sub_location,$loc_id)
    {
     
      if($main_location==$sub_location)
       {
        if($main_location==12  || $main_location==15)
        {
           $location=[0=>[12=>'ward_master',15=>'colony_master'],1=>[12=>'city_id',15=>'ward_id']];
          $sql=DB::table($location[0][$main_location])->where([[$location[1][$main_location],'=',$loc_id],['stat','=','A']])->get();
          $res = $sql->count();
